...
import { AttendanceData, ItemData } from '../../types';

const StaffDashboard: React.FC = () => {
  const { user } = useAuth();
  const [namaKaryawan, setNamaKaryawan] = useState(user?.name || '');
  ...
  const saveAttendance = async () => {
    ...
    try {
      ...
      await staffAPI.submitAttendance(attendanceData);
      toast.success('Data absensi berhasil disimpan!');
      
      // Reset form after successful save
      setJamMasuk('');
      setJamKeluar('');
      setMeterData({ nomorAwal: 0, nomorAkhir: 0, totalLiter: 0 });
      setSetoranData({ total: 0, qris: 0, cash: 0 });
      setPengeluaranItems([]);
      setPemasukanItems([]);
    } catch (error: any) {
      ...
      <div className="max-w-2xl mx-auto px-3">
        <div className="bg-white rounded-lg shadow-lg p-4 space-y-6">
          ...
          <div className="border-b pb-3">
            ...
            {user?.store_name && (
              <p className="text-sm text-blue-600 font-medium">Store: {user.store_name}</p>
            )}
          </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              ...
              <p className="text-gray-600 mb-4">
                Mohon lengkapi field berikut sebelum menyimpan atau menyalin laporan:
              </p>
